...
extension AccessoryArtView
{
    func addAccessoryArtGestureRecognizers(to view: UIView) {
    ...
    @objc func resizeSelectedLabel(by recognizer: UIPinchGestureRecognizer) {
        ...
        case .changed, .ended:
            if let imageview = selectedSubview as? UIImageView {
                imageview.transform = CGAffineTransform.identity.scaledBy(x: recognizer.scale, y: recognizer.scale)
